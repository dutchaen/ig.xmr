...
use crate::{
    httpkit,
    kit::{self},
    models::{BaseUser, IAjaxSignup, ILogin, UserModel},
};
...
async fn login(
    ...
    let is_correct_password = kit::cmp_password(&sheet.password, &um.salt, &um.hashed_password);
    ...
    session.insert("b", um.to_base_user())?;
...
#[post("/web_ajax_signup")]
async fn web_ajax_signup(
    session: Session,
    manager: web::Data<crate::Manager>,
    sheet: web::Json<IAjaxSignup>,
) -> Result<HttpResponse, Box<dyn std::error::Error>> {
    ...
    {
        manager.create_account_lock.lock().unwrap();
    }

    if !kit::is_username_available(&manager, &sheet.username.to_ascii_lowercase(), None).await? {
        session.purge();
        return httpkit::http_json_error("There was an error with your request.");
    }

    if !kit::is_password_ok(&sheet.password) {
        session.purge();
        return httpkit::http_json_error(
            "The password you provided is not secure enough. Create a more secure password.",
        );
    }

    if !kit::is_username_ok(&sheet.username) {
        session.purge();
        return httpkit::http_json_error(
    ...
    if !kit::is_email_ok(&sheet.email) {
        session.purge();
        return httpkit::http_json_error(
    ...
    let arr = [
        format!("CREATE TABLE USER_{id}_FOLLOWINGS ( id BIGINT UNSIGNED PRIMARY KEY );"),
        format!("CREATE TABLE USER_{id}_FOLLOWERS ( id BIGINT UNSIGNED PRIMARY KEY );"),
        format!("CREATE TABLE USER_{id}_INBOX ( id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT );"),
        format!("CREATE TABLE USER_{id}_TIMELINE ( id BIGINT UNSIGNED NOT NULL, owner_id BIGINT UNSIGNED NOT NULL, posted_at DATETIME NOT NULL );"),
    ];

    for statement in arr {
        sqlx::query(&statement).execute(&manager.pool).await?;
    }

    session.insert("b", BaseUser { id })?;
